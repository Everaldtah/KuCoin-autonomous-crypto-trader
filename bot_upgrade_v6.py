...
MAX_POSITION_PCT = float(os.environ.get("MAX_POSITION_PCT", "8.0"))  # UPGRADED: was 15.0 — reduced to 8% after analysis showed -0.69% EV per trade
MIN_POSITION_PCT = float(os.environ.get("MIN_POSITION_PCT", "2.0"))   # UPGRADED: was 3.0 — allows finer sizing
PORTFOLIO_DRAWDOWN_LIMIT = 8.0  # Emergency stop at 8% portfolio loss

# UPGRADED: Signal Thresholds — RAISED after backtest showed 0.55 fires too many false signals
# Analysis: 35.6% win rate at 0.55 threshold. Need 45%+ to break even.
# Raising to 0.65 should cut trade frequency ~50% and raise win rate to 45%+
BUY_SIGNAL_THRESHOLD = float(os.environ.get("BUY_SIGNAL_THRESHOLD", "0.65"))  # UPGRADED: was 0.55
SELL_SIGNAL_THRESHOLD = float(os.environ.get("SELL_SIGNAL_THRESHOLD", "0.35"))  # Keep at 0.35

# UPGRADED: Position sizing — reduced from 10% to 5% after analysis showed fee drag of 116%/year
# At £1,000 capital: £30/trade max, fee drag cut from £3.20/day to £1.60/day
POSITION_SIZE_PCT = float(os.environ.get("POSITION_SIZE_PCT", "5.0"))  # NEW: explicit position sizing control

# UPGRADED: Timeframe — upgrade from 1hr to 4hr to reduce noise and fee drag by ~75%
TIMEFRAME = os.environ.get("TIMEFRAME", "4hour")  # NEW: was hardcoded 1hour. 4hour = 75% fewer signals

# UPGRADED: Regime filter — add BTC market regime check
REGIME_FILTER_ENABLED = True  # NEW: halt new entries when BTC is in bearish regime
BTC_REGIME_EMA_FAST = 20  # 20-period EMA
BTC_REGIME_EMA_SLOW = 50  # 50-period EMA

# UPGRADED: Uncorrelated pairs — analysis showed 2 major clusters. Trade only uncorrelated pairs.
# From correlation analysis: ATOM is anti-correlated to BTC cluster (-0.97 with ETH)
# Limit to these + 1 from each cluster max
PREFERRED_PAIRS = [
    "ATOM-USDT",   # Anti-correlated to BTC cluster (-0.97 with ETH)
    "ADA-USDT",    # Independent / weakly correlated
    "AAVE-USDT",   # Anti-correlated to BTC (-0.18) and BTC cluster
    "DOGE-USDT",   # Anti-correlated to ADA (-0.94)
]
MAX_CORRELATED_PAIRS = 2  # Max pairs from the same correlation cluster
...
class SmartPortfolioTrader:
    ...
    async def _get_btc_regime(self) -> str:
        """
        UPGRADED: Detect BTC market regime using EMA crossover.
        Returns BULL (fast EMA > slow EMA), BEAR (fast < slow), or NEUTRAL.
        Used to halt new entries during bearish regime.
        """
        try:
            klines = await self.client.get_klines("BTC-USDT", TIMEFRAME, 100)
            if not klines or len(klines) < BTC_REGIME_EMA_SLOW + 1:
                return "NEUTRAL"
            closes = [float(k[2]) for k in klines]
            # Compute EMAs
            ema_fast_vals = EnsembleIndicators._ema(closes, BTC_REGIME_EMA_FAST)
            ema_slow_vals = EnsembleIndicators._ema(closes, BTC_REGIME_EMA_SLOW)
            if not ema_fast_vals or not ema_slow_vals:
                return "NEUTRAL"
            if ema_fast_vals[-1] > ema_slow_vals[-1]:
                return "BULL"
            elif ema_fast_vals[-1] < ema_slow_vals[-1]:
                return "BEAR"
            return "NEUTRAL"
        except Exception:
            return "NEUTRAL"
    ...
    async def _update_single_pair(self, symbol: str):
        ...
        try:
            ...
            # UPGRADED: use configurable TIMEFRAME (was hardcoded "1hour")
            klines = await self.client.get_klines(symbol, TIMEFRAME, 100)
            ...
            if active_count < MAX_PAIRS and pair.signal == "BUY" and not current_position:
                ...
                if size > 0 and size <= self.portfolio.available:
                    if not self.dry_run:
                        ...
                        print(f"[DRY] Would BUY {size:.2f} USDTDT {symbol} @ {pair.current_price:.2f}")
                        success = True
