...
# Signal Thresholds (ensemble) - UPGRADED v5.1
# Higher threshold for more selective entries (was 0.55)
BUY_SIGNAL_THRESHOLD = 0.65  # Composite score >= 65% for entry
SELL_SIGNAL_THRESHOLD = 0.35  # Composite score <= 35% for exit

# Risk Management - UPGRADED v5.1
# Better R:R ratio: 5% TP / 2% SL = 2.5:1 (was 3% / 1.5% = 2:1)
# This reduces breakeven win rate from 33% to 29%
TRAILING_STOP_PCT = 1.0  # Activate trailing at +1% profit
TAKE_PROFIT_PCT_BASE = 5.0  # UPGRADED from 3.0
STOP_LOSS_PCT_BASE = 2.0    # UPGRADED from 1.5
ATR_PERIOD = 14  # For volatility sizing

# Feature flags - NEW v5.1
TREND_ALIGNMENT_REQUIRED = True  # Require EMA/MACD/SuperTrend to agree
VOLUME_CONFIRMATION = True       # Require volume > 20-period average
BTC_REGIME_FILTER = True         # Only trade when BTC > 200 EMA
BTC_EMA_PERIOD = 200
MIN_ADX = 20.0  # Minimum trend strength for entries
...
class PairData:
    ...
    trend_aligned: bool = False  # NEW v5.1
...
class PortfolioState:
    ...
    indicator_weights: Dict[str, float] = field(default_factory=lambda: {
        "rsi": 0.15,  # v5.1: reduced from 0.20
        "ema": 0.25,  # v5.1: increased from 0.20 (trend following emphasis)
        "mfi": 0.10,  # v5.1: reduced from 0.15
        "macd": 0.20, # v5.1: increased from 0.15
        "bb": 0.10,   # v5.1: reduced from 0.15
        "super_trend": 0.15,  # v5.1: increased from 0.10
        "adx": 0.05
    })
...
class KuCoinAsyncClient:
    ...
    async def get_account(self) -> Dict:
        ...
        if success:
            accounts = {}
            # First pass: collect all trade accounts (where orders execute)
            for acc in data:
            ...
            # Fill in any currencies not found in trade accounts from main
            for acc in data:
...
class EnsembleIndicators:
    ...
    def compute_all(closes: List[float], highs: List[float], lows: List[float], 
        ...
        # 9. Volume metrics - NEW v5.1
        result["avg_volume_20"] = np.mean(volumes[-20:]) if len(volumes) >= 20 else np.mean(volumes)
...
class SmartPortfolioTrader:
    ...
    async def initialize(self):
        ...
        # Capital = available USDT + deployed capital (restored from state)
        self.portfolio.capital = float(usdt_available) + self.portfolio.deployed
    ...
    async def _update_btc_regime(self):
        """Check BTC trend regime (200 EMA filter) - NEW v5.1"""
        if not BTC_REGIME_FILTER:
            self.portfolio.btc_above_ema = True
            return
        try:
            klines = await self.client.get_klines("BTC-USDT", "1hour", 250)
            if len(klines) >= BTC_EMA_PERIOD:
                closes = [float(k[2]) for k in klines]
                ema200 = EnsembleIndicators._ema(closes, BTC_EMA_PERIOD)
                if ema200:
                    self.portfolio.btc_ema200 = ema200[-1]
                    self.portfolio.btc_above_ema = closes[-1] > ema200[-1]
                    regime = "BULL" if self.portfolio.btc_above_ema else "BEAR"
                    print(f"[REGIME] BTC: ${closes[-1]:.2f} | EMA{BTC_EMA_PERIOD}: ${ema200[-1]:.2f} | {regime}")
        except Exception as e:
            print(f"[WARN] BTC regime check failed: {e}")
            self.portfolio.btc_above_ema = True  # Default to allowing trades
    ...
    async def run(self):
        ...
        print(f"Buy Threshold: {BUY_SIGNAL_THRESHOLD} (was 0.55)")
        print(f"Take Profit: {TAKE_PROFIT_PCT_BASE}% (was 3.0%)")
        print(f"Stop Loss: {STOP_LOSS_PCT_BASE}% (was 1.5%)")
        print(f"BTC Regime Filter: {'ON' if BTC_REGIME_FILTER else 'OFF'}")
        print(f"Trend Alignment: {'ON' if TREND_ALIGNMENT_REQUIRED else 'OFF'}")
        ...
        while self.running:
            try:
                ...
                # 2. UPDATE BTC REGIME (NEW v5.1)
                await self._update_btc_regime()
                
                # 3. UPDATE ALL PAIRS (parallel)
                await self._update_all_pairs()
                
                # 4. RANK PAIRS by opportunity score
                ...
                self.portfolio.cycles_since_start += 1
                
                # Cycle summary - NEW v5.1
                cycle_pairs = len(self.portfolio.pairs)
                cycle_positions = len(self.portfolio.active_positions)
                cycle_pnl = self.portfolio.total_pnl
                btc_status = "BULL" if self.portfolio.btc_above_ema else "BEAR"
                print(f"[SUMMARY] Cycle {cycle} complete | Pairs: {cycle_pairs} | Positions: {cycle_positions} | "
                      f"Total P&L: ${cycle_pnl:+.2f} | BTC Regime: {btc_status}")
    ...
    async def _update_single_pair(self, symbol: str):
        ...
        try:
            ...
            # NEW v5.1: Check trend alignment (EMA, MACD, SuperTrend all agree)
            ema_bullish = indicators.get("ema_signal", 0) == 1
            macd_bullish = indicators.get("macd_histogram", 0) > 0
            st_bullish = indicators.get("super_trend", "NEUTRAL") == "BULLISH"
            pair.trend_aligned = ema_bullish and macd_bullish and st_bullish
            
            # Determine signal with v5.1 enhancements
            # NEW: Check ADX minimum threshold
            adx_ok = indicators.get("adx", 0) >= MIN_ADX
            
            # NEW: Check volume confirmation
            volume_ok = True
            if VOLUME_CONFIRMATION and pair.avg_volume_20 > 0:
                volume_ok = pair.current_volume > pair.avg_volume_20
            
            if composite >= BUY_SIGNAL_THRESHOLD and adx_ok and volume_ok:
                pair.signal = "BUY"
    ...
    def _rank_pairs(self) -> List[Tuple[str, float]]:
        ...
        for symbol, pair in self.portfolio.pairs.items():
            ...
            # NEW v5.1: Trend alignment bonus
            alignment_bonus = 1.3 if pair.trend_aligned else 1.0
            
            # NEW v5.1: Volume confirmation bonus
            volume_bonus = 1.2 if pair.current_volume > pair.avg_volume_20 else 1.0
            
            total = (volume_score * 0.2 + signal_strength * 0.3 + vol_score * 0.2 + adx_score * 0.3) * 100
            total *= alignment_bonus * volume_bonus
    ...
    async def _evaluate_positions(self, top_pairs: List[Tuple[str, float]]):
        ...
        for symbol, score in top_pairs:
            ...
            if current_position:
                ...
                # Update unrealized PnL tracking for health monitoring
                current_position["unrealized_pnl"] = (pnl_pct / 100) * current_position.get("size_usdt", 0)
            ...
            if active_count < MAX_PAIRS and pair.signal == "BUY" and not current_position:
                # NEW v5.1: BTC Regime Filter - skip entries in bear market
                if BTC_REGIME_FILTER and not self.portfolio.btc_above_ema:
                    print(f"[FILTER] Skipping {symbol} entry - BTC below EMA{BTC_EMA_PERIOD} (bear regime)")
                    continue
                
                # NEW v5.1: Trend Alignment Filter
                if TREND_ALIGNMENT_REQUIRED and not pair.trend_aligned:
                    print(f"[FILTER] Skipping {symbol} entry - trend indicators not aligned")
                    continue
                
                # NEW v5.1: ADX filter for entries
                if pair.adx < MIN_ADX:
                    print(f"[FILTER] Skipping {symbol} entry - ADX {pair.adx:.1f} < {MIN_ADX} (weak trend)")
                    continue
        ...
        try:
            if os.path.exists(STATE_FILE):
                with open(STATE_FILE, 'r') as f:
                    ...
                    self.portfolio.deployed = state.get("deployed", 0)
